...
def gre(balanced, ny, nz, ndummy):
    ...
    if balanced:
        dummy_segment = np.asarray([rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, delay])
        segment = np.asarray([rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay])
    else:
        dummy_segment = np.asarray([rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, spoil, delay])
        segment = np.asarray([rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, spoil, delay])
...
def test_find_segment_definitions(isbal, Ny, Nz):
    ...
    if isbal:
        npt.assert_allclose(segments[0], [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay])    
    else:
        npt.assert_allclose(segments[0], [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, spoil, delay])    
    ...
    if isbal:
        dummy = [rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, delay]
        dummy = np.concatenate([dummy for n in range(2)]).tolist()
        npt.assert_allclose(segments[0], dummy)    
        npt.assert_allclose(segments[1], [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay])    
    else:
        dummy = [rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, spoil, delay]
        dummy = np.concatenate([dummy for n in range(2)]).tolist()
        npt.assert_allclose(segments[0], dummy) 
        npt.assert_allclose(segments[1], [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, spoil, delay])    
    ...
    if isbal:
        dummy = [rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, delay]
        dummy = np.concatenate([dummy for n in range(3)]).tolist()
        npt.assert_allclose(segments[0], dummy) 
        npt.assert_allclose(segments[1], [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay])    
    else:
        dummy = [rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, spoil, delay]
    ...
    assert len(segments) == 1
    if isbal:
        readout = [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay]
        readout = np.concatenate([readout for n in range(Nz)]).tolist()
        npt.assert_allclose(segments[0], [prep, crush] + readout)    
    else:
        readout = [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, spoil, delay]
        readout = np.concatenate([readout for n in range(Nz)]).tolist()
        npt.assert_allclose(segments[0], [prep, crush] + readout)    
    ...
    if isbal:
        dummy = [rf, slice_reph, delay, phase_enc, freq_enc, read, freq_enc, phase_enc, delay]
        dummy = np.concatenate([dummy for n in range(Nz)]).tolist()
        readout = [rf, slice_reph, delay, phase_enc, freq_enc, read_adc, freq_enc, phase_enc, delay]
        readout = np.concatenate([readout for n in range(Nz)]).tolist()
