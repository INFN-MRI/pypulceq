...
def grad2ge(p, grad, sys, dt):
    ...
    if grad.type == "grad": # arbitrary or extended trap
        ...
        if np.all(np.abs(tt_rast - np.arange(1, len(tt_rast) + 1)) < 1e-6):
            ...
            area_in = np.sum((grad.waveform[:-1] + grad.waveform[1:]) / 2 * np.diff(grad.tt))
            wavtmp = grad.waveform
            tttmp = grad.delay + grad.tt
    else: # trap
        area_in = (grad.rise_time / 2 + grad.flat_time + grad.fall_time / 2) * grad.amplitude
    ...
    tt = np.arange(0.5 * sys.raster * 1e-6, tttmp[-1] + 0.5 * sys.raster * 1e-6, sys.raster * 1e-6)
    tmp = np.interp(tt, tttmp, wavtmp, left=0, right=0)

    area_out = np.sum(tmp) * sys.raster * 1e-6

    if np.any(np.isnan(tmp)):
        raise ValueError(f'NaN in gradient trapezoid waveform after interpolation (parent block {p})')

    if abs(area_in) > 1e-6:
        if abs(area_in - area_out) / abs(area_in) > 1e-4:
            warning_msg = (f'Gradient area not preserved after interpolating to GE raster time (in: {area_in:.3f} 1/m, out: {area_out:.3f}). '
                           'Did you wrap all gradient events in trap4ge?')
            warnings.warn(f'Warning: {warning_msg}')
...
def _trap2arb(grad):
    if grad.flat_time > 0:
        ...
        tt = grad.delay + np.asarray([0, grad.rise_time, grad.rise_time + grad.flat_time, grad.rise_time + grad.flat_time + grad.fall_time])
    else:
        wav = np.asarray([0, 1, 0]) * grad.amplitude
        tt = grad.delay + np.asarray([0, grad.rise_time, grad.rise_time + grad.fall_time])
