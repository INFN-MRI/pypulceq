...
def gradinterp(g, sysGE, seqGradRasterTime=10e-6, outRasterTime=4e-6):
    ...
    if g.type == "grad":
        ...
        if np.all(np.abs(tt_rast - np.arange(1, len(tt_rast) + 1)) < 1e-6):  
            ...
            tttmp = g.delay + np.concatenate(([0], g.tt, [g.tt[-1] + seqGradRasterTime / 2]))
        else:
    ...
    if np.abs(areaIn) > 1e-6:
        if np.abs(areaIn - areaOut) / np.abs(areaIn) > 1e-4:
            warning_msg = 'Gradient area not preserved after interpolating to GE raster time'
...
def trap2arb(g):
    ...
    if g.flat_time > 0:
        ...
        tt = np.array([g.delay, g.delay + g.rise_time, g.delay + g.rise_time + g.flat_time,
                       g.delay + g.rise_time + g.flat_time + g.fall_time])
    else:
        wav = np.array([0, 1, 0]) * g.amplitude
        tt = np.array([g.delay, g.delay + g.rise_time, g.delay + g.rise_time + g.fall_time])
...
def trap4ge(gin, commonRasterTime, sys):
    ...
    """
    gout = pp.make_trapezoid(gin.channel, 
                             system=sys, 
