...
def preflightcheck(entryFile, seqstampFile, sysGE):
    ...
    with open(seqstampFile, 'w') as fout:
        ...
        with open(entryFile, 'r') as fid:
            ...
            b1CheckFile = fid.readline().strip()  # .mod file used for b1 scaling and SAR checks, e.g., 'tipdown.mod'
            readoutFilterFile = fid.readline().strip()  # .mod file used to set the receive filter, e.g., 'readout.mod'
        ...
        for mod in modArr:
            ...
            if mod['hasDAQ'] and hdr['rfres'] != ndaq:
                raise ValueError(f"Number of samples in {readoutFilterFile} and {mod['fname']} do not match (must be the same across all .mod files containing ADC windows)")
            if hdr['b1max'] != b1limit:
                raise ValueError(f"B1 limit in {mod['fname']} does not match {entryFile} (must be the same across all .mod files)")
        ...
        for mod in modArr:
            ...
            is_valid, gmaxtmp, slewmaxtmp = utils.checkwaveforms(sysGE, gx=gx, gy=gy, gz=gz, rfUnit="G", gradUnit="G/cm", slewUnit="G/cm/msec")
        ...
        for tStart in range(0, 1000, 5):
            ...
            print(msg, end='')
            rf, gx, gy, gz, tRange = utils.plotseq(sysGE, timeRange=[int(tStart), int(tStart)+10])

            maxEnergy = max(maxEnergy, sum(np.abs(rf)**2) * sysGE.raster * 1e-6)  # Gauss^2 * s
        ...
        P_ref = sysGE.raster * 1e-6 * E_ref / TR_ref  # RF power (Gauss^2) of the reference scan
        P_this = maxEnergy / tSpan  # Peak 10s average RF power (Gauss^2 of the current scan)
        SAR_predicted = P_this / P_ref * 6.2  # W/kg

        # Print various parameters to the file
        fout.write(f'{TRequiv}\n')
        fout.write(f'{round(powerx)}\t{round(powery)}\t{round(powerz)}\n')  # Needed in minseq() in the .e file
