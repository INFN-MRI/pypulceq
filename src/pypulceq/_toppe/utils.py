...
def padwaveforms(rf=(), gx=(), gy=(), gz=()):
    ...
    if rf.shape[0] != 0:
        if int((rf != 0).sum()) == 0:
            warnings.warn('(First) RF waveform contains all zeros -- ignored', UserWarning)
    ...
    assert ndat <= 2**15, f'Max waveform length is 32768 (samples) -- found {ndat} samples'
...
def checkwaveforms(system, rf=(), gx=(), gy=(), gz=(), rfUnit="uT", gradUnit="mT/m", slewUnit="T/m/s"):
    """
    ...
    for ax in axes:
        ...
        if gmax_tmp > maxGrad:
            print(f"Error: {ax} gradient amplitude exceeds system limit {gmax_tmp / maxGrad * 100}\n")
            isValid = False
        gmax.append(gmax_tmp)
        
        smax_tmp = abs(np.diff(g[ax], axis=0) / (system.raster * 1e3)).max(axis=0).max()
        if smax_tmp > maxSlew:
            print(f"Error: {ax} gradient slew rate exceeds system limit {smax_tmp / maxSlew * 100}\n")
            isValid = False
    ...
    if len(rf) > 0:
        if max(abs(rf.flatten())) > maxRF:
        	print(f'Error: rf amplitude exceeds system limit {max(abs(rf)) / maxRF * 100}\n')
        	isValid = False
        else:
            isValid = True
    else:
...
def plotseq(sysGE, timeRange, loop=None, loopFile='scanloop.txt', moduleListFile='modules.txt', timeOnly=False):
    """
...
def _sub_getwavs(blockStart, blockStop, loop, modules, sysGE, dur):
    ...
    # loop over modules
    rho, theta, gx, gy, gz = _sub_getwavs_nb(irho, itheta, igx, igy, igz, ia_rf, dtheta, ia_gx, ia_gy, ia_gz, Rv, res, w)
...
# @nb.njit(cache=True)
def _sub_getwavs_nb(irho, itheta, igx, igy, igz, ia_rf, dtheta, ia_gx, ia_gy, ia_gz, Rv, res, w):
