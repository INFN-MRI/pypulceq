...
def writemod(system, rf=(), gx=(), gy=(), gz=(), ofname='out.mod', desc='TOPPE module', nomflip=90.0, hdrfloats=(), hdrints=(), nChop=(0, 0), rfUnit='uT', gradUnit='mT/m', slewUnit='T/m/s', **kwargs):
    """
    ...
    assert nChop[1] % 4 == 0 and nChop[1] % 4 == 0, 'Each entry in nChop must be multiple of 4'
    ...
    paramsint16 = [nChop[0], rf.shape[0]-sum(nChop)] # these reserved ints are used by interpreter
...
def writemodfile(modlist, sysGE, ignoreTrigger=False):
    # Write modules.txt
    ...
    with open('modules.txt', 'w') as fid:
        ...
        for k, b in modlist.items():
            ...
            dur = rf.shape[0] * sysGE.raster + sysGE.rfRingdownTime # us
...
def _myrfstat(b1, nom_fa, system):
    ...
    hardpulse = max(abs(b1)) * np.ones_like(b1)  # hard pulse of equal duration and amplitude
...
def _writemod(fname, desc, rf, gx, gy, gz,  paramsint16, paramsfloat, system):
    ...
    with open(fname, 'wb') as f:    
        ...
        fs = inspect.stack()
        if len(fs) > 2:
        	globaldesc = f'{globaldesc}called by (dbstack(3)): {fs[2][1].split(sep)[-1]}\n'
        ...
        np.array(ncoils).astype(np.int16).byteswap().tofile(f) # shorts must be written in binary -- otherwise it won't work on scanner 
        np.array(res).astype(np.int16).byteswap().tofile(f)
        np.array(npulses).astype(np.int16).byteswap().tofile(f)
        f.write(f'b1max:  {float(system.maxRF)}\n'.encode("utf-8")) # (floats are OK in ASCII on scanner)
